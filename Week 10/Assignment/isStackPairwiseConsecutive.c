...
{
    /* add your code here */
    int item1, item2, diff;

    if (!isEmptyStack(s)) {     // if stack is not empty
        if ((s->ll.size) % 2 == 0) {    // if there is even numbers in the stack
            while (!isEmptyStack(s)) {
                item1 = pop(s);
                item2 = pop(s);
                diff = item1 - item2;
                if (!(diff == 1 || diff == -1)) {   // if diff not equal 1 or diff not equal -1
                    return 0;
                }
            }
        }
        else {  // if there is odd numbers in the stack
            return 0;
        }
    }
