...
void removeOddValues(Queue *q)
{
	/* add your code here */
    int i, item;
	ListNode *last;	// last points to last in queue, temp points to first queue

    if (q->ll.head != NULL) {	// if q is not empty
		last = findNode(&(q->ll), (q->ll.size) - 1);
		while (q->ll.head != last) {	// enqueue/dequeue until head == last
		...
		// enqueue/dequeue when head == last
		if ((q->ll.head->item) % 2 == 0) {
			item = q->ll.head->item;
			dequeue(q);
			enqueue(q, item);
