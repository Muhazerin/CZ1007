...
{
    /* add your code here */
    ListNode *max, *current, *previous, *previousMax;

    if (*ptrHead == NULL) { // if the list is empty
        return 0;
    }
    else {
        max = current = *ptrHead;
        previous = previousMax = NULL;
        while (current != NULL) {
        ...
        if (previousMax != NULL) {  // this mean max is not the first node
            previousMax->next = previousMax->next->next;
            max->next = *ptrHead;
            *ptrHead = max;
        }
    }
