...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
    ...
    if (ll1->size <= ll2->size) {   // if ll1 size <= ll2 size, merge until cannot
        while (ll1Index <= ll1->size) {
            insertNode(ll1, ll1Index, ll2->head->item);
            removeNode(ll2, 0);
            ll1Index += 2;
        }
        return;
    }
    else {  // ll1 size > ll2 size, just merge
        temp = ll2->head;
        while (temp != NULL) {
            insertNode(ll1, ll1Index, temp->item);
            temp = temp->next;
            removeNode(ll2, 0);
            ll1Index += 2;
        }
