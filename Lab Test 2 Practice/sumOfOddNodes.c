...
int sumOfOddNodes(BTNode *node)
{
    ...
    if (node == NULL) { // if the tree is empty or it encounters null node in the tree
        return 0;
    }
    else {
        sum += sumOfOddNodes(node->left);  // traverse left side
        sum += sumOfOddNodes(node->right); // traverse right side
        if ((node->item % 2) != 00) {
            sum += node->item; 
        }
        return sum;
