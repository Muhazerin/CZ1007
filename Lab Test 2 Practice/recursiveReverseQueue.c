...
void recursiveReverse(Queue *q)
{
    ...
    if (q->ll.head == NULL || q->ll.size == 1) {   // if empty queue or only have 1 item inside
        return;
    }
    if (q->ll.size == 2) {  // if queue have 2 items inside (base case)
        enqueue(q, dequeue(q));
    }
    else {  // more than 2 items inside (recursive case)
        restQ.ll.head = q->ll.head->next;
        restQ.ll.size = q->ll.size - 1;
        recursiveReverse(&restQ);
        q->ll.head->next = restQ.ll.head;
        enqueue(q, dequeue(q));
    }
