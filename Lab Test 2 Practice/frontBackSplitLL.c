...
{
	/* add your code here */
    int halfwayPointIndex, index;
    ListNode *temp;

    if (ll->head == NULL) { // if ll is empty
        return;
    }

    halfwayPointIndex = (ll->size - 1) / 2;
    index = 0;
    while (index <= ll->size - 1) {
        temp = findNode(ll, index);
        if (index <= halfwayPointIndex) {   // add to frontList
            insertNode(resultFrontList, resultFrontList->size, temp->item);
        }
        else {  // add to backList
