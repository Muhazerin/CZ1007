...
{
	/* add your code here */
    if (node == NULL) { // if tree is empty or encounters null node
        return;
    }
    else {
        if (node->item < m) {
        ...
        printSmallerValues(node->left, m);  // traverse left side
        printSmallerValues(node->right, m); // traverse right side
    }
}
