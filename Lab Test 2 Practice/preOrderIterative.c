...
void preOrderIterative(BSTNode *root)
{
    ...
    else {  // got something inside tree
        current = root;
        push(&s, current->right);
        printf("%d ", current->item);
        current = current->left;
        while (s.top != NULL || current != NULL) {  // do this when stack is still not empty or current is not null
            while (current != NULL) {
                printf("%d ", current->item);   // print it
                push(&s, current->right);       // push right side
