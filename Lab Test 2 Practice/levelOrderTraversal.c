...
void levelOrderTraversal(BSTNode* root)
{
    /* add your code here */
	BSTNode *tempNode;
	Queue temp;			// creating new queue
	temp.head = NULL;
	temp.tail = NULL;
	

	if (root == NULL) {	// if tree is empty or encounters null node
		return;
	}
	else {
		enqueue(&(temp.head), &(temp.tail), root);
		while (temp.head != NULL) {
			tempNode = dequeue(&(temp.head), &(temp.tail));
			printf("%d ", tempNode->item);
			if (tempNode->left != NULL) {	// only enqueue if there is left node
				enqueue(&(temp.head), &(temp.tail), tempNode->left);	// enqueue left node
			}
			if (tempNode->right != NULL) {	// only enqueue if there is right node
				enqueue(&(temp.head), &(temp.tail), tempNode->right);	// enqueue left node
			}
		}
	}
}
